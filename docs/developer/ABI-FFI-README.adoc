...
# HARVARD_DEHALLUCINATOR ABI/FFI Documentation
...
│  generated/abi/harvard_dehallucinator.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libharvard_dehallucinator.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── harvard_dehallucinator.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── harvard_dehallucinator.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/harvard_dehallucinator.h
```
...
```c
#include "harvard_dehallucinator.h"

int main() {
    void* handle = harvard_dehallucinator_init();
    if (!handle) return 1;

    int result = harvard_dehallucinator_process(handle, 42);
    if (result != 0) {
        const char* err = harvard_dehallucinator_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    harvard_dehallucinator_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lharvard_dehallucinator -L./zig-out/lib
```

### From Idris2

```idris
import HARVARD_DEHALLUCINATOR.ABI.Foreign
...
```rust
#[link(name = "harvard_dehallucinator")]
extern "C" {
    fn harvard_dehallucinator_init() -> *mut std::ffi::c_void;
    fn harvard_dehallucinator_free(handle: *mut std::ffi::c_void);
    fn harvard_dehallucinator_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = harvard_dehallucinator_init();
        assert!(!handle.is_null());

        let result = harvard_dehallucinator_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libharvard_dehallucinator = "libharvard_dehallucinator"

function init()
    handle = ccall((:harvard_dehallucinator_init, libharvard_dehallucinator), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:harvard_dehallucinator_process, libharvard_dehallucinator), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:harvard_dehallucinator_free, libharvard_dehallucinator), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/harvard_dehallucinator.h
   ```
