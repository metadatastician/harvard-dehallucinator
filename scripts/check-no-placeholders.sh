...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "harvard-dehallucinator"`
...
# an instantiation. The template submitted the literal `harvard-dehallucinator` as its own
...
# is set to `owner/harvard-dehallucinator` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `harvard-dehallucinator-2`,
